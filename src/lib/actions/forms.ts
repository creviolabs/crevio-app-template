...
function formDataValidator(data: unknown): FormData {
	if (!(data instanceof FormData)) {
		throw new Error("submitForm expects FormData");
	}
	return data;
}

/** The Form's schema, for rendering its fields. */
...
export const submitForm = createServerFn({ method: "POST" })
	...
	.handler(async ({ data: formData }): Promise<SubmitFormState> => {
		...
		try {
			...
			await crevio.formSubmissions.create({ formId, email, name, answers });
